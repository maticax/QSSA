...
Initials = Init;
%Init_qssa = [Initials(1) beta*Initials(1)*Initials(3)*exp(-1)...
 %  /(beta*Initials(3)*exp(-1)+delta)...
  %  .*(1-exp(-(Initials(3)*beta*exp(-1)+delta)./c))]; 
%Init_qssa = [Initials(1) beta*Initials(1)*(Initials(3)*exp(-1)+pp*epsilon)/c]; 
